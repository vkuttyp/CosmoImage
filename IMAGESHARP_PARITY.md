...
| `Configuration` registry | Global + per-image `Configuration` — registers decoders/encoders/allocator | 🟡 `VipsConfiguration.Default` (rounds 89-92) — global registry seeded with all 18 built-in formats; `VipsIdentify.LoadAsync` is a single registry walk. User-registered customs override built-ins on sniff conflict. Per-instance `new VipsConfiguration()` (rounds 92) auto-seeds built-ins; `new VipsConfiguration(seedBuiltIns: false)` starts empty for sandboxed loading. `LoadAsync(stream, configuration)` overloads on both `VipsIdentify` and `VipsImageOps` for scoped registrations that don't pollute `Default`. Allocator registration still deferred |
