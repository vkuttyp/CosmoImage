...
| `Configuration` registry | Global + per-image `Configuration` — registers decoders/encoders/allocator | 🟡 `VipsConfiguration.Default` (rounds 89-91) — global registry seeded with all 18 built-in formats (PNG / JPEG / WebP / GIF / TIFF / BMP / QOI / HEIF / JXL / JP2K / PDF / SVG / HDR / FITS / NIfTI / MAT / PNM / TGA, registered in priority order so reverse-walk hits strongest magic first). `VipsIdentify.LoadAsync` is now a single registry walk — no more hardcoded switch. User-registered customs override built-ins for the same magic. `Reset()` restores defaults; individual `Unregister(format)` lets users disable specific built-ins. Per-image config + allocator registration deferred |
