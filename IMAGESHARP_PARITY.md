...
| Text rendering with full glyph shaping (via `SixLabors.Fonts`) | ✅ HarfBuzz-equivalent shaping, ligatures, kerning, RTL/LTR/BiDi | 🟡 round 72 added `VipsTextOps.DrawText` / `TextToPath` backed by `SixLabors.Fonts` — proper kerning, ligatures, basic OpenType features; glyph outlines emitted as `VipsPath` segments and filled via the existing scanline rasteriser. Round 73 added multi-line layout (`\n`, `WrappingLength`, `LineSpacing`, `HAlign`, `WordBreak`, `Justify`). Round 75 added `Decorations` (Underline / Strikeout / Overline as flags), `LayoutMode` (HorizontalTopBottom / HorizontalBottomTop / Vertical*), and `TextDirection` (LeftToRight / RightToLeft / Auto). Advanced OpenType (stylistic sets, alt forms) deferred to later rounds. Legacy `VipsImageOps.Text` (Magick.NET label render) still available for one-shot label generation |
