...
| ICC profile structure parsing | ✅ — `IccProfile` with header + tag table | 🟡 `VipsIccProfile` (round 85) — typed header (class, data / connection color space, version, creation datetime, primary platform, manufacturer / model / creator, rendering intent, PCS illuminant XYZ, profile ID, flags, attributes) + tag-table indexed by 4-char signature with raw tag-data round-trip. Header fields editable; tag-data parsing (desc / wtpt / curves / LUT-AtoB / etc.) deferred. Bridge methods `VipsImage.GetIccProfileTyped()` / `SetIccProfileTyped()` |
