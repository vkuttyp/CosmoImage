...
| Text rendering with full glyph shaping (via `SixLabors.Fonts`) | ✅ HarfBuzz-equivalent shaping, ligatures, kerning, RTL/LTR/BiDi | 🟡 round 72 added `VipsTextOps.DrawText` / `TextToPath` backed by `SixLabors.Fonts` — proper kerning, ligatures, basic OpenType features; glyph outlines emitted as `VipsPath` segments and filled via the existing scanline rasteriser. Round 73 added multi-line layout: explicit `\n` breaks, `WrappingLength`, `LineSpacing`, `HAlign` (Left / Center / Right within wrap box), `WordBreak` (Standard / BreakAll / KeepAll / BreakWord), and `Justify` (None / InterWord / InterCharacter). BiDi resolution and advanced OpenType (stylistic sets, alt forms) deferred to later rounds. Legacy `VipsImageOps.Text` (Magick.NET label render) still available for one-shot label generation |
| Text on path | ✅ | ❌ |
