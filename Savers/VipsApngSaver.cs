...
/// <summary>
/// Animated PNG (APNG) writer. Same multi-frame metadata convention as
/// <see cref="VipsGifSaver"/> and the animated <see cref="VipsWebpSaver"/>:
/// image height is N × page-height, with <c>n-pages</c>, <c>page-height</c>,
/// and optional <c>animation-delays</c> in <see cref="VipsImage.Metadata"/>.
/// Falls back to single-frame PNG when the multi-frame metadata isn't set.
/// </summary>
public static class VipsApngSaver
{
    public static async Task SaveAsync(VipsImage image, PipeWriter writer, CancellationToken cancellationToken = default)
    {
        ...
        for (int p = 0; p < nPages; p++)
        {
            ...
            frame.Format = MagickFormat.APng;
            if (nPages > 1) frame.AnimationDelay = delays[p];
            collection.Add(frame);
        }

        // Profiles attach to the first frame — typical animated-PNG convention.
        ...
        collection.Write(ms, MagickFormat.APng);
        ms.Position = 0;
